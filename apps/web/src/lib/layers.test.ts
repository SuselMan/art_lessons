...
describe('eraseThroughTargets', () => {
  ...
  // (#518) A folder's padlock reaches the layers inside it, so this has to
  // leave them alone — and it does so without knowing the rule, because it
  // filters on isLayerLocked rather than on the flags. The layer's own row
  // shows a closed lock it cannot open, which is the answer to "why did the
  // eraser skip this one".
  it('leaves out layers inside a locked folder', () => {
    const state = stateOf({
      f: folder('f', ['inside'], { locked: true }),
      inside: layer('inside'),
      loose: layer('loose'),
    }, ['f', 'loose'])

    expect(eraseThroughTargets(state)).toEqual(['loose'])
  })
