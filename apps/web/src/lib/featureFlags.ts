...
export const FEATURE_FLAGS: readonly FeatureFlagDef[] = [
  ...
  {
    key: 'markerLegacyStamps',
    label: 'Маркер: старый штамповый растеризатор (dev)',
    description: 'Возвращает маркеру прежний путь отрисовки — независимые отпечатки наконечника с мягким краем в долях радиуса (#330). Включать только для сравнения со штатной лентой: на большой кисти штамповый путь даёт видимые фестоны, а ширина края растёт вместе с размером кисти.',
  },
  {
    key: 'interfaceSound',
