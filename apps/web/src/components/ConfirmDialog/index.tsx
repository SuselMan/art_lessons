...
function ConfirmDialogView({ pending, onSettle }: {
  ...
  const acceptLabel = pending.kind === 'confirm'
    ? pending.request.confirmLabel ?? t('common.continue')
    : pending.request.dismissLabel ?? t('common.ok')
  ...
  return (
    <Modal
      ...
      ariaLabel={pending.request.title
        ?? t(pending.kind === 'confirm' ? 'dialog.confirmLabel' : 'dialog.noticeLabel')}
    ...
    >
      ...
      <div className={styles.actions}>
        {pending.kind === 'confirm' && (
          ...
          >
            {pending.request.cancelLabel ?? t('common.cancel')}
          </button>
        )}
        <button
          type="button"
