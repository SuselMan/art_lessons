...
export class DabSystem {
  constructor({ spacingFactor = 0.22 } = {}) {
    this.spacingFactor = spacingFactor;
    this._reset();
  }

  _reset() {
    this._buf = []; // rolling buffer, max 4 control points [P0, P1, P2, P3]
    this._remainder = 0;
  }

  // Returns first dab; subsequent segment rendering is deferred by 1 event.
  startStroke(x, y, pressure, tiltX, tiltY, baseSize) {
    this._reset();
  ...
  continueStroke(x, y, pressure, tiltX, tiltY, baseSize) {
  ...
  endStroke(baseSize) {
    ...
    const p3 = { x: 2 * p2.x - p1.x, y: 2 * p2.y - p1.y, pressure: p2.pressure, tiltX: p2.tiltX, tiltY: p2.tiltY };
  ...
  _splineDabs(p0, p1, p2, p3, baseSize) {
    // Arc-length lookup table for uniform dab spacing along the curve
    const STEPS = 16;
    const samples = [{ t: 0, len: 0, x: p1.x, y: p1.y }];
    let totalLen = 0;
  ...
  _makeDab(x, y, pressure, tiltX, tiltY, baseSize, pathAngle) {
